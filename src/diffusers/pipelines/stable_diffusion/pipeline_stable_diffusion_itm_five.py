...
def generate_sinusoid_tensor_from_latent(latent, frequency=1.0, amplitude=1.0):
    """
    根据输入的 latent 张量生成一个形状为 [N, 1, H, W] 的正弦信号张量，并与 latent 张量保持一致的设备。

    参数:
    - latent: 输入的 latent 张量，用于确定形状和设备
    - frequency: 正弦信号的频率（默认值为1.0）
    - amplitude: 正弦信号的幅度（默认值为1.0）

    返回:
    - sinusoid_tensor: 生成的正弦信号张量，形状为 [N, 1, H, W]，设备与 latent 张量相同
    """
    # 从 latent 张量中获取 N, H, W 参数
    ...
    # 生成正弦信号，应用幅度参数
    sinusoid = amplitude * torch.sin(frequency * xx)

    # 将正弦信号扩展到 [N, 1, H, W] 形状
    sinusoid_tensor = sinusoid.unsqueeze(0).unsqueeze(0).repeat(N, 1, 1, 1)
...
class FeatureFusion(nn.Module):
    ...
    def initialize_weights(self):
        with torch.no_grad():
            ...
            # 设置偏置项为 1/5，使得初始权重为 1/5
            self.weight_conv.bias.data.fill_(1/5)
    
    def forward(self, feature_1, feature_2, feature_3, feature_4, feature_5):
        ...
        weight4 = self.weight_conv(feature_4)
        weight5 = self.weight_conv(feature_5)        
        # 将每个通道的权重映射合并
        weights = torch.stack([weight1, weight2, weight3, weight4, weight5], dim=2)  # (N, C, 3, H, W)
...
class StableDiffusionITMFivePipeline(
    DiffusionPipeline,
    ...
    def encode_prompt(
        ...
        # print('lora_scale is', lora_scale)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        latents_3 = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
            width,
        ...
        latents_4 = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
            width,
        ...
        # 7. Denoising loop expoure_medium
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        self._num_timesteps = len(timesteps)
        # print('len(timesteps) is', len(timesteps))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_2 = torch.concat((latents_2, latents), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_2] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents_2 = self.scheduler.step(noise_pred, t, latents_2, **extra_step_kwargs, return_dict=False)[0]
                ...
                if callback_on_step_end is not None:
                    ...
                    latents_2 = callback_outputs.pop("latents_2", latents_2)
                    prompt_embeds = callback_outputs.pop("prompt_embeds", prompt_embeds)
                    negative_prompt_embeds = callback_outputs.pop("negative_prompt_embeds", negative_prompt_embeds)
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    ...
                    if callback is not None and i % callback_steps == 0:
                        step_idx = i // getattr(self.scheduler, "order", 1)
                        callback(step_idx, t, latents_2)


        # 9. Denoising loop exposure_3
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        self._num_timesteps = len(timesteps)
        # print('len(timesteps) is', len(timesteps))
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_3 = torch.concat((latents_3, latents_2), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_3] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents_3 = self.scheduler.step(noise_pred, t, latents_3, **extra_step_kwargs, return_dict=False)[0]
                ...
                if callback_on_step_end is not None:
                    ...
                    latents_3 = callback_outputs.pop("latents_3", latents_3)
                    prompt_embeds = callback_outputs.pop("prompt_embeds", prompt_embeds)
                    negative_prompt_embeds = callback_outputs.pop("negative_prompt_embeds", negative_prompt_embeds)
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    ...
                    if callback is not None and i % callback_steps == 0:
                        step_idx = i // getattr(self.scheduler, "order", 1)
                        callback(step_idx, t, latents_3)



        # 10. Denoising loop exposure_4
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        self._num_timesteps = len(timesteps)
        # print('len(timesteps) is', len(timesteps))
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_4 = torch.concat((latents_4, latents_3), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_4] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents_4 = self.scheduler.step(noise_pred, t, latents_4, **extra_step_kwargs, return_dict=False)[0]
                ...
                if callback_on_step_end is not None:
                    ...
                    latents_4 = callback_outputs.pop("latents_4", latents_4)
                    prompt_embeds = callback_outputs.pop("prompt_embeds", prompt_embeds)
                    negative_prompt_embeds = callback_outputs.pop("negative_prompt_embeds", negative_prompt_embeds)
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    ...
                    if callback is not None and i % callback_steps == 0:
                        step_idx = i // getattr(self.scheduler, "order", 1)
                        callback(step_idx, t, latents_4)



        if not output_type == "latent":
            
            pretrained_model_path = "/HPS/RawDiff/work/LED/vae_five/checkpoint-60000/merge_model.pth"#"/sensei-fs/users/wchao/Codes/diffusers/resutls/sd_model/StabelDiffusion_weights/vae/merge_model.pth"
            ...
            with torch.no_grad():
                latent_merged, weights_map = net(latents_4, latents_3, latents_2, latents, latents_tensor)
                # for i in range(4):
                    # for j in range(5):  # 对应 feature_1, feature_2, feature_3 的三个权重图
            ...
            # latens_scaling_np = (latent_merged / self.vae.config.scaling_factor).squeeze(0).detach().cpu().numpy()
            # name_tmp = img_name.replace('png', 'npy')
            # np.save(name_tmp, latens_scaling_np)

            # latens_scaling_h = (latents_tensor / self.vae.config.scaling_factor).squeeze(0).detach().cpu().numpy()
            ...
            # np.save(f"{name_tmp.replace('.npy', '_l_shift.npy')}", latens_scaling_l)
            # latent_merged =  (latents_tensor + latents + latents_low)/3.0
            image = self.vae.decode(latent_merged / self.vae.config.scaling_factor, return_dict=False, generator=generator)[
                0
            ]
            image_0 = self.vae.decode(latents_tensor / self.vae.config.scaling_factor, return_dict=False, generator=generator)[
                0
            ]
            latent = latents / self.vae.config.scaling_factor
            image_1 = self.vae.decode(latent, return_dict=False, generator=generator)[
                0
            ]
            latent_2= latents_2 / self.vae.config.scaling_factor
            image_2 = self.vae.decode(latent_2, return_dict=False, generator=generator)[
                0
            ]
            latent_3= latents_3 / self.vae.config.scaling_factor
            image_3 = self.vae.decode(latent_3, return_dict=False, generator=generator)[
                0
            ]
            latent_4= latents_4 / self.vae.config.scaling_factor
            image_4 = self.vae.decode(latent_4, return_dict=False, generator=generator)[
                0
            ]



            image  = torch.concatenate((image_tensor, image, image_0, image_1, image_2, image_3, image_4), dim = 0)
