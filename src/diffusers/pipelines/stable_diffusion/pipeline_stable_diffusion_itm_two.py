...
class FeatureFusion(nn.Module):
    ...
    def initialize_weights(self):
        with torch.no_grad():
            ...
            self.weight_conv.bias.data.fill_(1/2)
    
    def forward(self, feature_1, feature_3):
        ...
        # 将每个通道的权重映射合并
        weights = torch.stack([weight1, weight3], dim=2)  # (N, C, 3, H, W)
...
class StableDiffusionITMTWOPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not output_type == "latent":
            ...
            pretrained_model_path = "/HPS/RawDiff/work/LED/output_GANE_new_weights_disc_10_two/checkpoint-60000/merge_model.pth"#"/sensei-fs/users/wchao/Codes/diffusers/resutls/sd_model/StabelDiffusion_weights/vae/merge_model.pth"
            ...
            with torch.no_grad():
                latent_merged, weights_map = net(latents, latents_tensor)
                # for i in range(4):
            ...
            # latens_scaling_h = (latents_tensor).squeeze(0).detach().cpu().numpy()
            ...
            # np.save(f"{name_tmp.replace('.npy', '_l_re_ldr.npy')}", latens_scaling_l)
            # # latent_merged =  (latents_tensor + latents + latents_low)/3.0
            # image = self.vae.decode(latent_merged, return_dict=False, generator=generator)[
            #     0
            # ]
            # image_high = self.vae.decode(latents_tensor, return_dict=False, generator=generator)[
            #     0
            # ]
            # image_medium = self.vae.decode(latents, return_dict=False, generator=generator)[
            #     0
            # ]
            # image_low = self.vae.decode(latents_low, return_dict=False, generator=generator)[
            #     0
            # ]
            image  = torch.concatenate((image_tensor, image, image_high, image_medium), dim = 0)
