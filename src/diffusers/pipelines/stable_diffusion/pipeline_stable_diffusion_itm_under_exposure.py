...
class StableDiffusionITMUNPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not output_type == "latent":
            ...
            with torch.no_grad():
                latent_merged, weights_map = net(latents_tensor, latents, latents_low)
            ...
            latents_scaling_high = (latents_tensor ).squeeze(0).detach().cpu().numpy()
            # np.save(npy_save_name.replace('_2.npy', '_1.npy'), latents_scaling_high)
            image_medium = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False, generator=generator)[
                0
            ]
            latents_scaling_medium = (latents ).squeeze(0).detach().cpu().numpy()
