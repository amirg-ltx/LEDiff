...
class StableDiffusionPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_tensor = preprocess_image('/HPS/RawDiff/work/LED/over_exposed_images/single_hdr_5_2.png') #preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/IMG_6568.JPG', device = device)#preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/P1011321.JPG', device = device)#
        
        image_tensor_01 = image_tensor*0.5 + 0.5

        image_tensor_01_2 = ((image_tensor_01**2)*(1/8))
        image_tensor_01_2_norm = image_tensor_01_2*2.0 - 1.0

        image_tensor_01_1 = ((image_tensor_01**2)*(1/64))
        image_tensor_01_1_norm = image_tensor_01_1*2.0 - 1.0

        latents_tensor = self.vae.encode(image_tensor).latent_dist.mode() * self.vae.config.scaling_factor

        latents_tensor_2 = self.vae.encode(image_tensor_01_2_norm).latent_dist.mode() * self.vae.config.scaling_factor
        latents_tensor_1 = self.vae.encode(image_tensor_01_1_norm).latent_dist.mode() * self.vae.config.scaling_factor
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_1 = torch.concat((latents_tensor_2, latents, latents_tensor), dim = 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_1 = torch.concat((latents_tensor_1, latents_low, latents_medium), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_1] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
