...
class StableDiffusionPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_tensor = preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/IMG_6568.JPG', device = device)#preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/P1011321.JPG', device = device)#
        ...
        latents_tensor_first = latents_tensor[:, 0:1, :, :]
        positive_tensor = latents_tensor_first + 5.0#4.0 * torch.ones((latents.shape[0], 1, 64, 64), device=latents.device, dtype=latents.dtype)
        negative_tensor = latents_tensor_first - 5.0#0.25 * torch.ones((latents.shape[0], 1, 64, 64), device=latents.device, dtype=latents.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_1 = torch.concat((latents, latents_tensor, negative_tensor), dim = 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_1 = torch.concat((latents_low, latents_tensor, positive_tensor), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_1] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
        ...
        if not output_type == "latent":
            ...
            pretrained_model_path = "/sensei-fs/users/wchao/Codes/diffusers/resutls/sd_model/StabelDiffusion_weights/vae/merge_model.pth"
            ...
            with torch.no_grad():
                latent_merged, weights_map = net(latents, latents_tensor, latents_low)
