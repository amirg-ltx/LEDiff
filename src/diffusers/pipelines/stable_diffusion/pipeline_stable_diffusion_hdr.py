...
class StableDiffusionHDRPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        height = 512#height or self.unet.config.sample_size * self.vae_scale_factor
        width = 512#width or self.unet.config.sample_size * self.vae_scale_factor
        ...
        # latents_np = np.load('/HPS/RawDiff/work/LED/T2I_LDR/latent_code_LDR_3.npy')#np.load('/sensei-fs/users/wchao/Codes/diffusers/resutls/test_visualization_rand_render_test_ave/latent_codes_srgb_comparisons_night_scene_cars_5.npy')
        # latents_tensor = torch.from_numpy(latents_np).float().to(device).unsqueeze(0) * self.vae.config.scaling_factor
        
        latents_np = np.load(npy_name)#np.load('/HPS/RawDiff/work/LED/T2I_LDR/latent_code_LDR_3.npy')#np.load('/sensei-fs/users/wchao/Codes/diffusers/resutls/test_visualization_rand_render_test_ave/latent_codes_srgb_comparisons_night_scene_cars_5.npy')
        latents_tensor = torch.from_numpy(latents_np).float().to(device).unsqueeze(0) * self.vae.config.scaling_factor
        # latents_tensor = latents_tensor_sq[0].unsqueeze(0)#latents_tensor.view(1, 4, 64, 64*8)
        # image_tensor = preprocess_image('/HPS/RawDiff/work/LED/under_exposed_images/_MDF0002.png') #preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/IMG_6568.JPG', device = device)#preprocess_image('/sensei-fs/users/wchao/Codes/Ours/over_exposed_images/P1011321.JPG', device = device)#
        ...
        # print('latents_tensor_sq shape is', latents_tensor_sq.shape)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_concat_1 = torch.concat((latents_low, latents_medium), dim = 1)
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latent_concat_1] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
        ...
        if not output_type == "latent":
            ...
            image_medium = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False, generator=generator)[
                0
            ]
            image_low = self.vae.decode(latents_low / self.vae.config.scaling_factor, return_dict=False, generator=generator)[
                0
            ]
            # image  = torch.concatenate((image_tensor, image, image_high, image_medium, image_low), dim = 0)
            image  = torch.concatenate((image_high, image, image_high, image_medium, image_low), dim = 0)
